...
func (g *Game) Run() error {
	...
	for !rl.WindowShouldClose() {
		...
		rl.DrawRectangle(screenWidthMargin, 0, platformScreenWidth, screenHeight, rl.NewColor(30, 30, 30, 255))
		state.player.Draw()
		for _, l := range state.levels {
			l.draw()
		}
		rl.DrawText(
			fmt.Sprintf("Vel: %.4f, %.4f", state.player.vel.X, state.player.vel.Y),
			10,
			10,
			21,
			rl.White,
		...
		rl.DrawText(
			fmt.Sprintf("Pos: %.4f, %.4f", state.player.pos.X, state.player.pos.Y),
			10,
			40,
			21,
			rl.White,
